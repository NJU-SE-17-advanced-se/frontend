  ...
  methods: {
    ...
    async fetchSearchResult(keyword: string, page = 1) {
      ...
      try {
        ...
        // 页面切换时数量不同，先清空
        if (affiliationIds.length !== this.affiliationsBasicInfo.length) {
          this.affiliationsBasicInfo = [];
        }
        // 实现分批加载时的响应
        res1.forEach((info, i) => {
          this.$set(this.affiliationsBasicInfo, i, info.data);
        });
        res2.forEach((info, i) => {
          this.$set(this.affiliationsBasicInfo, 4 + i, info.data);
        });
        res3.forEach((info, i) => {
          this.$set(this.affiliationsBasicInfo, 7 + i, info.data);
        });
