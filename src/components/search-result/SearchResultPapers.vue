    ...
    async fetchSearchResult(
      ...
      if (!start) {
        ...
        try {
          ...
          // 页面切换时数量不同，先清空
          if (paperIds.length !== this.papersBasicInfo.length) {
            this.papersBasicInfo = [];
          }
          // 实现分批加载时的响应
          res1.forEach((info, i) => {
            this.$set(this.papersBasicInfo, i, info.data);
          });
          res2.forEach((info, i) => {
            this.$set(this.papersBasicInfo, 4 + i, info.data);
          });
          res3.forEach((info, i) => {
            this.$set(this.papersBasicInfo, 7 + i, info.data);
          });
